...
def ustvari_json(sez):
    ...
    for j in sez:
        ...
        rrhtml = BeautifulSoup(rr.text, 'html.parser')
        ...
        if(len(tr_ji) != 4):
            ...
            Tip = Znamka_in_tip[len(Znamka):]  
            prva_registracija = re.findall(r'\d{1,4}',str(tr_ji[1]))
            leto_proizvodnje = re.findall(r'\d{4,4}',str(tr_ji[2]))
            if len(re.findall("Leto",str(tr_ji[2]))) == 0:
                tr_ji.insert(2, '')
                leto_proizvodnje = "unknown"
            #tuki vmes se lahko certifikat prkaze
            if(len(re.findall(r'\d{1,}',str(tr_ji[3])))>1):
                del tr_ji[3]
            prevozeni_kilometri =  re.findall(r'\d{1,}',str(tr_ji[3]))
            if len(re.findall(r'\d{1,4}',str(tr_ji[4]))) != 2:
                tr_ji.insert(4, '')
            tehnicni_pregled = re.findall(r'\d{1,}',str(tr_ji[4]))
            ...
            thisdict = {
                ...
                "veljavnost tehnicnega pregleda" : tehnicni_pregled,
...
p = ustvari_json(sez)
p = json.dumps(p)   
with open("all_data_avotnet.json",'w') as f:
	f.write(p)
	f.close()
